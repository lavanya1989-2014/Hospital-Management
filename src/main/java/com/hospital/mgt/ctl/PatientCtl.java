...
public class PatientCtl extends BaseCtl {
	...
	public String display(HttpSession session,@RequestParam(required = false) Long id, @ModelAttribute("form") PatientForm form, Model model) {
		...
		PatientDTO pDto=patientservice.findByUserId(uDto.getId());
		if(pDto!=null) {
			form.populate(pDto);
		}else {
			form.setFirstName(uDto.getFirstName());
			form.setLastName(uDto.getLastName());
			form.setDob(DataUtility.getStringDate(uDto.getDob()));
			form.setPhoneNo(uDto.getPhoneNo());
			form.setGender(uDto.getGender());
		}
		return "Patient";
	...
	public String submit(Locale locale,HttpSession session, @RequestParam String operation, @ModelAttribute("form") @Valid PatientForm form,
